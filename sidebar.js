...
var menuItems = Array.from(document.querySelectorAll(".menu_item"));

function scrollToSubject(itemIndex) {
  console.log(window.innerHeight * itemIndex, $(window).scrollTop());
  if (window.innerHeight * itemIndex != $(window).scrollTop()) {
    console.log(window.innerHeight * itemIndex);
    window.scrollTo({
      left: 0,
      top: window.innerHeight * itemIndex,
      behavior: "smooth"
    });
  } else {
    return;
